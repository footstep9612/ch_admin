...
function edit()
{
    ...
    if (IS_AJAX) {
        ...
        if ($nodeModel->save()) {
            //清除缓存
            $log = \Lib\Cache::clear(\Lib\Cache::LABEL_RULE_CONTROLLER_ACTION,$nodeModel->controller . $nodeModel->action);
            logs(sprintf("清除缓存[%s] ID[%s]",json_encode($log),$id),'debug-clear-rbac');
