...
{   
   ...
   if (empty($password)) {
        ajaxReturn(array('error' => 100, 'msg' => '密码不能为空'));
   }
   $password = md5($password);
   $sessionObj = new Session(); 
   $adminId = $sessionObj->get('userData.admin_user.userid');
   $adminUserModel = new Model\AdminUser($adminId);
   if($adminUserModel->password == $password){
       ajaxReturn(array('error' => 100, 'msg' => '新密码和旧密码相同'));   
   } 
   ...
   if ($adminUserModel->save()) {
        ...
        ajaxReturn(array('error' => 100, 'msg' => '密码重置失败'));
   }
}