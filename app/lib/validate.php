...
/**
 * @字段自动验证类库
 * @参考https://github.com/Wixel/GUMP
 * @author zhangzhijie
 * Class Validaterules
 */
namespace Lib;
use App\common\IdCard;
...
class Validate
{
    ...
    protected $lang = array(
        'validate_required'                 => '该 {field} 字段值不能为空',
        'validate_valid_email'              => '该 {field} 字段值必须为合法的邮箱地址',
        'validate_max_len'                  => '该 {field} 字段值最多不能超过{param}个字符',
        'validate_min_len'                  => '该 {field} 字段值最少不能低于{param}个字符',
        'validate_exact_len'                => '该 {field} 字段值必须等于{param}个字符',
        'validate_alpha'                    => '该 {field} 字段值只能包含英文字母',
        'validate_alpha_numeric'            => '该 {field} 字段值只能包含英文字母或数字',
        'validate_alpha_numeric_space'      => '该 {field} 字段值只能包含英文字母，数字或下划线',
        'validate_alpha_dash'               => '该 {field} 字段值只能包含英文字母或下划线',
        'validate_alpha_space'              => '该 {field} 字段值只能包含英文字母或空格',
        'validate_numeric'                  => '该 {field} 字段值只能包含数字',
        'validate_integer'                  => '该 {field} 字段值必须为整数类型',
        'validate_boolean'                  => '该 {field} 字段值必须为布尔类型',
        'validate_float'                    => '该 {field} 字段值必须为浮点型(float)',
        'validate_valid_url'                => '该 {field} 字段值必须为合法的URL',
        'validate_url_exists'               => '该 {field} 字段值必须为合法且存在的URL地址',
        'validate_valid_ip'                 => '该 {field} 字段值必须为合法的IP地址',
        'validate_valid_ipv4'               => '该 {field} 字段值必须包含一个合法的IPV4地址',
        'validate_valid_ipv6'               => '该 {field} 字段值 必须包含一个合法的IPV6地址',
        'validate_guidv4'                   => '该 {field} 字段值必须为合法的全局唯一',
        'validate_valid_cc'                 => '该 {field} 字段值必须为合法的信用卡地址',
        'validate_valid_name'               => '该 {field} 字段值必须为合法的用户名',
        'validate_contains'                 => '该 {field} 只能为其中的字符: {param}',
        'validate_contains_list'            => '该 {field} 不是有效的选项',
        'validate_doesnt_contain_list'      => '该 {field} 字段值不被允许',
        'validate_street_address'           => '该 {field} 字段值必须为合法的道路地址',
        'validate_date'                     => '该 {field} 字段必须为有效的日期格式',
        'validate_min_numeric'              => '该 {field} 字段必须为数值类型且不能小于{param}',
        'validate_max_numeric'              => '该 {field} 字段必须为数值类型且不能大于 {param}',
        'validate_min_age'                  => '该 {field} 要求年龄不能低于{param}',
        'validate_invalid'                  => '该 {field} 验证未通过',
        'validate_starts'                   => '该 {field} 字段需要以{param}开始',
        'validate_extension'                => '该 {field} 字段只能为以下扩展: {param}',
        'validate_required_file'            => '该 {field} 字段不能不能为空',
        'validate_equalsfield'              => '该 {field} 字段值必须等于 {param}',
        'validate_iban'                     => '该 {field} 字段必须为合法的国际银行卡账号',
        'validate_phone_number'             => '该 {field} 必须为合法的电话号码',
        'validate_regex'                    => '该 {field} 正则验证未通过',
        'validate_valid_json_string'        => '该 {field} 字段需要为一个合法的JSON字符',
        'validate_valid_array_size_greater' => '该 {field} 必须为数组类型，且必须最少有 {param} 元素',
        'validate_valid_array_size_lesser'  => '该 {field} 必须为数组类型，且必须最多有 {param} 元素',
        'validate_valid_array_size_equal'   => '该 {field} 必须为数组类型，且必须有 {param} 元素',
        'validate_valid_id_card'            => '该 {field} 不是有效的身份证格式',
    );


    // ** ------------------------- Validation Helpers ---------------------------- ** //

    public function __construct(array $lang = array())
    {
        if (!empty($lang))
        {
            $this->lang = $lang;
    ...
    public static function setLabel(array $label = array())
    {
        self::$label = $label;
    }
     ...
     * @throws Exception
     */
    public function validate(array $input, array $ruleset,$group = '')
    {
        $this->errors = array();
        ...
        foreach ($ruleset as $field => $rules) {
            ...
            if (count(array_intersect($lookFor, $rules)) > 0 || (isset($input[$field]) && !is_array($input[$field]))) {
                //对于不存在的字段，如果设置required 或者 required_file 设置错误信息
                if (!isset($input[$field]) && count(array_intersect($lookFor, $rules)) > 0)
                {
                    continue;
                }
    ...
    public function get_readable_errors($convert_to_string = false, $field_class = 'gump-field', $error_class = 'gump-error-message')
    {
        ...
        foreach ($this->errors as $e) {
            if (isset(self::$label[$e['field']]) && !empty(self::$label[$e['field']]))
            {
                $field = self::$label[$e['field']];
            }else
            {
     ...
     *
     * @param string $field
     * @param string $input date 可以用 date_parse()正确解析的日期字符串
     * @param null   $param
     *
     * @return mixed
     */
    protected function validate_date($field, $input, $param = null)
    {
        ...
        if (date_parse($input[$field]) === false)
        {
            return array(
                'field' => $field,
                'value' => $input[$field],
     ...
     * @param null $param
     */
    protected function validate_valid_id_card($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }
        if (!(new IdCard())->validateIDCard($input[$field])){
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule'  => __FUNCTION__,
