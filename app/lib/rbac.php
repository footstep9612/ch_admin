...
class Rbac{
   			...
   			foreach ($value['child'] as $k => $val) {
   				if(strtolower($controller)==$val['controller']){
   					for ($i=0; $i <count($value['child']) ; $i++) {
   						if($value['child'][$i]['action']==strtolower($action)){
   							$auth=1;
   						}
   					}
   				}
    ...
    public function getNodeIdByGroupId($group_id){
    	...
    	logs($roleModel->getLastQuery(), "sql");
    	$ruleString='';
    ...
    private  function getNodeInfoByRule($rule){
        ...
        $getNodeList=$nodeModel->whereIn('id',$rule)->get()->resultArr();
        if($getNodeList){
	 ...
	 * @return bool
	 */
	public function checkRoleRbac($roleId,$controller,$action)
	{
		$getNodeModel = self::getModelByTableName(C('RBAC_NODE'));
		$nodeModel = new $getNodeModel;
		//获取rule_id
		...
		$ruleInfo = $nodeModel->where(array('controller' => $controller,'action' => $action,'status' => 1))->get()->resultArr();
		//不存在的控制器和方法 不给权限
		if (empty($ruleInfo)) {
			return false;
		}
		$getRoleName = self::getModelByTableName(C('RBAC_ROLE'));
		$roleModel = new $getRoleName;
		$roleInfo = $roleModel->where(array('id' => $roleId,'status' => '1'))->get()->rowArr();
		//role不存在 不给权限
		if (empty($roleInfo)) {
			return false;
		}
		$rule = explode(',',$roleInfo['rule']);
		$rule = array_unique($rule);
		foreach ($ruleInfo as $row) {
			if (in_array($row['id'],$rule)) {
				return true;
			}
		}
		return  false;
	 ...
	 * @param $action
	 */
	public function getNodeIdByControllerAction($controller,$action)
	{
		if (empty($controller) || empty($action)) {
			return false;
		}
		$key = $controller . $action;
		$data = Cache::getArray(Cache::LABEL_RULE_CONTROLLER_ACTION,$key);
		...
		if (empty($data)) {
			...
			$list = $nodeModel
					->fields('id')
					->where(array('controller' => $controller,'action' => $action,'status' => 1))
					->get()
					->resultArr();
			...
			Cache::setArray(Cache::LABEL_RULE_CONTROLLER_ACTION,$key,$data);
		}
		return $data;
	}
	 ...
	 * @return bool
	 */
	public function getRuleByRoleId($roleId)
	{
		if (empty($roleId)) {
			return false;
		}
		...
		$roleInfo = $roleModel
				->fields('rule')
				->where(array('id' => $roleId,'status' => '1'))
				->get()
				->rowArr();
		if ($roleInfo) {
	 ...
	 * @return bool
	 */
	public function checkActionPermission(session $session, $controller, $action)
	{
		...
		$aId = $this->getNodeIdByControllerAction($controller,$action);
		if (!$aId) {
			return false;
		}
		$ruleList = $session->get('userData.admin_user');

		if (empty($ruleList) || !isset($ruleList['rule_list']) || empty($ruleList['rule_list'])) {
			return false;
		}
		$ruleList = explode(',',$ruleList['rule_list']);
		$ruleList = array_filter($ruleList);
		return !empty(array_intersect($ruleList,$aId));
	}
}
